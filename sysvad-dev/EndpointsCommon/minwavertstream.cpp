...
// MoDi: Global virtual cable pointer (render→capture passthrough)
CVirtualCable* g_pVirtualCable = NULL;
...
--*/
{
    ...
    // MoDi: Initialize shared virtual cable buffer (idempotent, only first call allocates)
    if (!g_pVirtualCable)
    {
        g_pVirtualCable = new (NonPagedPoolNx, 'CVM') CVirtualCable();
        if (g_pVirtualCable) g_pVirtualCable->Init();
    }
...
)
{
    ...
    // MoDi: Zero-fill DMA buffer to prevent garbage data (beep noise) when no app is playing
    if (m_pDmaBuffer) RtlZeroMemory(m_pDmaBuffer, RequestedSize_);
...
)
{
    ...
    if (m_bCapture)
    {
        // MoDi: Read from virtual cable (render→capture passthrough)
    ...
    else
    {
        ...
        // MoDi: Write render data to virtual cable for capture passthrough
        if (g_pVirtualCable)
        {
            ULONG cbToCopy = ByteDisplacement;
            ULONG offset = m_ullLinearPosition % m_ulDmaBufferSize;
            while (cbToCopy > 0)
            {
                ULONG chunk = min(cbToCopy, m_ulDmaBufferSize - offset);
                g_pVirtualCable->Write(m_pDmaBuffer + offset, chunk);
                offset = (offset + chunk) % m_ulDmaBufferSize;
                cbToCopy -= chunk;
            }
        }
    }
...
This function fills the capture DMA buffer from the virtual cable
(render-to-capture passthrough). Falls back to silence if no data.
...
--*/
{
    ...
    // MoDi: Read from shared virtual cable buffer instead of ToneGenerator
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        if (g_pVirtualCable)
            g_pVirtualCable->Read(m_pDmaBuffer + bufferOffset, runWrite);
        else
