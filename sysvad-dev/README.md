...
> 许可边界：本目录基于 Microsoft Windows Driver Samples 的 SysVAD 示例，保留 Microsoft 原有版权与署名，并按 Microsoft Public License（MS-PL）分发。Silvite 在本目录中的修改同样置于 MS-PL；本目录不是 GPL-3.0-or-later 应用核心的一部分。完整许可见 `../LICENSES/MS-PL.txt`。
